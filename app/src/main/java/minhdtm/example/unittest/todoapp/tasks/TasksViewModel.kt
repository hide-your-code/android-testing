 ...
 */
@HiltViewModel
class TasksViewModel @Inject constructor(private val tasksRepository: DefaultTasksRepository) : ViewModel() {
    ...
    private val _items: LiveData<List<Task>> = _forceUpdate.switchMap { forceUpdate ->
        ...
        // Return live data
        tasksRepository.observeTasks().switchMap { result ->
            liveData {
                if (result is Success) {
                    emit(filterItems(result.data, currentFiltering))
                } else {
                    emit(emptyList())
